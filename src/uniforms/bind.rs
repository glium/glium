        ...
        where P: ProgramExt
    {
        ...
        self.visit_values(|name, value| {
            ...
            if let Some(uniform) = program.get_uniform(name) {
                ...
                let fence = match bind_uniform_block(&mut ctxt,
                                                     &value,
                                                     block,
                                                     program,
                ...
                let fence =
                    match bind_shared_storage_block(&mut ctxt,
                                                    &value,
                                                    block,
                                                    program,
...
fn bind_uniform_block<'a, P>(ctxt: &mut context::CommandContext,
...
fn bind_shared_storage_block<'a, P>(ctxt: &mut context::CommandContext,
                                    value: &UniformValue<'a>,
                                    block: &program::UniformBlock,
                                    program: &P,
                                    buffer_bind_points: &mut Bitsfield,
    ...
    where P: ProgramExt
{
    match value {
        &UniformValue::Block(buffer, ref layout) => {
            ...
            program.set_shader_storage_block_binding(ctxt,
                                                     block_id,
          ...
          T: TextureExt
{
    ...
    let texture_unit = ctxt.state
                           ...
                           .unwrap_or_else(|| {
                               texture_bind_points.get_unused()
    ...
    if ctxt.state.texture_units[texture_unit as usize].texture != texture.get_texture_id() ||
       ctxt.state.texture_units[texture_unit as usize].sampler != sampler {
        ...
        if ctxt.state.texture_units[texture_unit as usize].sampler != sampler {
            ...
            unsafe {
                ctxt.gl.BindSampler(texture_unit as gl::types::GLenum, sampler);
            }
            ctxt.state.texture_units[texture_unit as usize].sampler = sampler;
        }
    }
