...
use std::io::Cursor;
use glium::{Display, Surface, texture::{Texture2d, SrgbTexture2d}};
use glutin::surface::WindowSurface;
use support::{ApplicationContext, State, view_matrix};

struct Application {
    pub shape: glium::VertexBuffer<Vertex>,
...
impl ApplicationContext for Application {
    const WINDOW_TITLE:&'static str = "Glium tutorial #14";
    ...
    fn draw_frame(&mut self, display: &Display<WindowSurface>) {
        ...
        let t = (std::time::Instant::now() - self.start).as_secs_f32() * 2.0;
        let ang = t.sin();
